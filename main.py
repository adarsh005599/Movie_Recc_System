...
async def lifespan(app: FastAPI):
    ...
    print("--- Loading Pickle Resources ---")
    try:
        ...
        print(f"ERROR: Could not find pickle files. {e}")
        # In production, you might want to raise an error here
    
    yield
    # Clean up (shutdown) logic goes here if needed
    print("--- Shutting Down ---")
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    if not TMDB_API_KEY:
        raise HTTPException(status_code=500, detail="TMDB_API_KEY is missing in .env")
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"TMDB API Error: {e.response.text}")
    except httpx.RequestError:
        raise HTTPException(status_code=502, detail="Network error reaching TMDB")
...
@app.get("/health")
def health():
    return {"status": "ok", "resources_loaded": df is not None}
